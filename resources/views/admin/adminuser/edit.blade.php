    ...
    <el-form label-position="top" label-width="80px" :model="formdata" :rules="rules" ref="formdata">
        ...
        <el-form-item label="分配角色" prop="role" :error="errormsg.role">
            <el-select v-model="formdata.role" placeholder="请选择角色">
                <el-option v-for="item in roles" :key="item.id" :label="item.show_name" :value="item.id">
                </el-option>
            </el-select>
        </el-form-item>
            ...
            submitForm: function(formName) {
                ...
                this.$refs[formName].validate(function(valid) {
                    if (valid) {
                        ...
                        vm.errormsg = {};
                        setTimeout(function() {
                            axios.put("/{{config('webset.web_indexname')}}/adminuser/{{$id}}", vm.formdata).then(function(reason) {
                                if (reason.data.code == 0) {
                                    ...
                                    setTimeout(function() {
                                        window.location.href = "{{ route('admin.adminuser.index') }}";
                                    }, 1000);
                                } else {
                                    vm.$message({
                                        type: 'error',
